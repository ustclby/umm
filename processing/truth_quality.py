...
for fn in file_names:
    with h5py.File(fn, 'r') as h5_file:
        ...
        index = np.random.choice(len(data), select_num,replace=False) # choose a random subset with size select_num
        cnt = cnt + len(index)
        for i in index:
            print(i)
            ...
            img_name = '/home/umm/img/' + info[i][0:-4] + '_from_' + str(int(labels[i])) + '_to_' + str(pre) + 'with_prob_' + str(predicts[pre]) +'.jpg'
            print(img_name)
            cv2.imwrite(img_name, img)

print('accuracy = ' + str(float(cnt-cntw)/float(cnt)))
print('end')
